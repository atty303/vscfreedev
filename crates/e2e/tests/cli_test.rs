...
async fn test_ssh_connection() -> Result<()> {
    // Build the remote server binary first
    let mut build_cmd = Command::new("cargo");
    build_cmd
        .arg("build")
        .arg("--bin")
        .arg("vscfreedev_remote");
    ...
    // Run the CLI with SSH connection
    let mut cmd = Command::cargo_bin("vscfreedev")?;
    cmd.arg("ssh")
        .arg("--host")
        .arg("127.0.0.1")
        .arg("--port")
    ...
    cmd.assert()
        ...
        .stdout(predicates::str::contains("Sending message: Hello from E2E test!"))
        .stdout(predicates::str::contains("Echo: Hello from E2E test!"))
        .stdout(predicates::str::contains("SSH connection test completed successfully"));
