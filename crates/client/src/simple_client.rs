...
use crate::simple_client_transport::{
    SimpleYuhaClientTransport, connect_local, connect_ssh_transport,
};
use crate::transport::{LocalTransport, SshTransport, TransportConfig};

/// Type alias for backward compatibility
pub type SimpleYuhaClient<T> = SimpleYuhaClientTransport<T>;
...
) -> Result<SimpleYuhaClient<SshTransport>, ClientError> {
...
/// Connect to a local process and return a SimpleYuhaClient
pub async fn connect_local_process(
    binary_path: Option<&Path>,
) -> Result<SimpleYuhaClient<LocalTransport>, ClientError> {
